...
mod tests {
    ...
    fn the_committed_corpus_is_discovered_and_scored_by_nothing() {
        ...
        // `hostname-collision.toml` (story 4.15) two and `docker-veth.toml` (story 4.16) two —
        // twenty-one in the committed corpus.
        assert_eq!(report.discovered(), 21, "the walk must open the corpus");
        assert_eq!(
            report.scored(),
            0,
    ...
    fn the_report_says_plainly_that_nothing_was_scored() {
        ...
        assert!(rendered.contains("21 trap(s) discovered"), "{rendered}");
    ...
    fn a_trap_with_no_answer_is_discovered_but_not_scored() {
        ...
        // A correct answer for one trap, so `scored` is 1 while `discovered` stays 21.
        answers.insert(
        ...
        assert_eq!(report.discovered(), 21);
