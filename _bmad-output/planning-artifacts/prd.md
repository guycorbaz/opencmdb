  ...
  - date: '2026-07-16'
    ...
    changes: |
      ...
      1 ADDITION - a canonical vocabulary section (binding). accept-as-declared -> document / accept-gap.
        The UI label is "Add" / « Ajouter » since v0.3.1; "merge" is retired in English because the
...
- FR13: The operator can **document** a discovery in one action — writing the observed values into the declared record — in the two distinct cases the loop actually produces:
  ...
  - **UI terminology (FR/EN):** the interface names this gesture **"Add" / « Ajouter »**; documentation, API and code use **`document`** (`document-field`, `document-all`). 🔴 **This is the ONE concept whose UI label differs from its identifier**, and the reason is what the action does: the engine can only ADD a declared value — there is no overwrite path in the adapter and the primary key refuses a second write for one field (NFR5). *The label read « Merger » until v0.3.1; it was an anglicism for « Fusionner », and « fusionner » names an operation this product forbids, exactly as `merge` does in English.* ⚠️ A human asking the product to CHANGE a value they themselves declared is a different and legitimate gesture, which does not exist yet — Epic 21's edit.
...
| Resolving inbox items | **triage** | **triage** | document / accept-gap / create / attach / **exclude** / snooze / **resolve** |
