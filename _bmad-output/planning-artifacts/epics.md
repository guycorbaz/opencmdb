...
⚠️ **DECOMPOSED 2026-09-11 into FOUR stories (see the full section below), and the four do NOT close it**: FR21's VLAN half and FR25 (IPv6) are in scope and outside the 2026-09-10 arbitrations. Its close is not readable off the story count. ✅ **Updated by Epic 14's (partial) retrospective, 2026-09-19**: the four became SEVEN by three splits (14.2b, 14.3a, 14.4b), all `done`; and Guy chose to deliver the rest before closing — **14.5 (VLANs, FR21's other half) and 14.6 (IPv6, observation-only, FR25)**, preceded by a tooling story 14.4c the retrospective owes. ✅ **CLOSED by the FINAL retrospective, 2026-09-22: TEN stories delivered, all `done`** — 14.1, 14.2, 14.2b, 14.3a, 14.3b, 14.4, 14.4b, **14.4c**, 14.5, 14.6. 🔑 **14.4c is COUNTED, and its provenance is why this sentence exists** (Guy's decision 2, 2026-09-22): it was created BY the partial retrospective as its own action items A1 and A2, so it appears in no story list written before it. ⚠️ Leaving a tooling story outside the epic was REFUSED as the way a count nobody can reconcile gets published — this line said *nine* while ten had shipped, and the discrepancy was found by the final retrospective rather than by any gate. The epic closes only after the PROJECT REVIEW that follows this retrospective.
...
_✅ **ARBITRATED 2026-09-26 by Guy** (story file §0.8, §0.10), and it DIVERGES from this note: **no device is stored and no membership table is written** — the grouping is computed at read time from the operator's current `match` rows, which are the source. The divergence from 6.12's *"a record rather than a computation"* and from D15 case A is registered, owner the first consumer needing a stable device id (Epic 15, FR26). A record reaches its interface by its declared MAC, else its ORIGIN observation (the address path merged two machines by DHCP, measured); only answers join two interfaces; the header names *appareils* and *fiches*._
...
_**Decomposed 2026-09-11 with Guy, on six arbitrations taken 2026-09-10 before any story was written.** Four stories _(seven by 2026-09-19 — 14.2b, 14.3a and 14.4b were split off at validation or implementation — then **TEN delivered**: 14.4c, a tooling story the partial retrospective created as its own action items, plus 14.5 VLANs and 14.6 IPv6, all decided by Guy at that retrospective)_. ⚠️ **The epic is NOT closed by them**: FR21's VLAN half and FR25 (IPv6, observation-only) are in scope and outside the arbitrations, and a term minted before its screen exists is minted by accident. They are named here so the epic's close is not read off the story count._
