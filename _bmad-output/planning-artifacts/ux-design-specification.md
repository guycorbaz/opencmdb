...
| **Lift the doubt of an ambiguity** | **resolve** | **résoudre** | The gesture of the `ambiguous` state: the operator says whether the candidates the engine refused to guess between are **one machine or several** — two answers of one gesture, *the same machine* and *distinct machines*. Each is kept as an OPERATOR decision the engine never supersedes (D14); *the same machine* is what the device grouping reads (story 6.14c). Named before it acted (story 6.14), it acts since story 6.14b. It is **not** `attach`: no record exists to attach to, and the answer is symmetric. ⚠️ **Not a conflict's gesture**: two sources disagreeing about one field are answered by *document* at field level. ⚠️ FR33 also lists *resolve* among an alert's actions; that use is the alerts epic's to reconcile with this row |
