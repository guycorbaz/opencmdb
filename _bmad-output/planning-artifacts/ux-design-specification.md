...
editHistory:
  ...
  - date: '2026-07-17'
    ...
    changes: |
      F58 (D57-scope) - THE IMPACT VIEW LEAVES THE MVP. 5 sites.
        THE RULE behind it, and it is the PM's, adopted: a relationship the product may one day OBSERVE is
        a DECLARATION; a relationship nothing will ever observe is a BELIEF. The MVP ships declarations.
        Growth ships beliefs the day an observer exists to contradict them.
        `depends_on`/`connects_to` + the traversal -> GROWTH, beside the interactive graphical topology -
        WHERE THIS SPEC HAD ALREADY PUT IT. `hosts`/`exposes` (containments) stay.
        At MVP the migration-prep job is answered by the device record's "Hosted here" panel: one
        containment hop, and FR39 already carried it - "view its full record (declared attributes,
        observation history, connection point, HOSTED APPLICATIONS)". Marc's Sunday question - "what is
        running on this box before I power it down?" - was already in the MVP and nobody had read it.
        🔴 AND THE WORD "IMPACT" IS REFUSED ON A ONE-HOP SCREEN. A view named Impact that concludes
        "nothing else is affected" is FR7's `satisfaction` trap with a nav entry: present, well-typed and
        wrong is more dangerous than missing - the missing one fails loudly. And WORSE than `satisfaction`,
        which is observed and can be contradicted by a connector: `depends_on` is the only field in the
        product with no loop behind it. It can only rot, and nothing is watching.
        Sites: the desktop-context line · the signature-screens line · the screen-hierarchy line
        ("supporting, not signature") · "The confident shutdown" · the Growth list.
      `X` FOR EXCLUDE - WITHDRAWN by Guy. See the note appended under F47 below: it is recorded, not
        deleted (annulment is an addition, D21). Two reasons, and the second dissolves the question:
        (1) `X` applied the criterion that had just died - a MNEMONIC - when this very entry states that
            `E` was never one. The criterion is MUSCLE MEMORY OF THE CORPUS. And on that criterion `X` is
            likely a collision: in Gmail, `x` SELECTS.
        (2) There was no binding to replace. VERIFIED against the body: the letters E/G/S/I appear NOWHERE
            in this spec. It fixes gestures and says "kbd hints" exist; it never assigns a letter. `I` was
            never specified - the editHistory line "keyboard bindings E/G/S/I" described an addition to a
            list that does not exist.
        ==> The non-subject is now DOCUMENTED IN THE BODY (Keyboard flow), not only in this journal: this
            spec assigns no letters BY DECISION; the letters are written with the inbox screen, all six
            gestures at once, against the corpus. The dividend, which is the real point: BINDINGS ARE
            DECOUPLED FROM OUR VOCABULARY - a rename can never orphan a key again.
        (F56, 4th instance in 24h: reasoning from the map instead of the terrain - the orchestrator's
         twice over, having written the journal line and then built a standing question on it.)
  - date: '2026-07-16'
    source: 'architecture workflow (steps 5-8) + the cardinality probe run against the reference device — the UX half of items F42, F44, F45, F46, F47, F50, F51'
    changes: |
      F46 (ADDITION — a design that did not exist, not a line to fix): "Source State: Two Axes, and Only
        One of Them Is a Colour". This spec had designed LIVENESS (live/blind, the frozen banner) and had
        NO design for CAPABILITY at all. The cell that creates - `Live + Reduced`, a source that answers
        perfectly and simply sees less - was undesigned, and it is the one the operator meets most often
        after the happy path.
        THE RULE: `Blind` is an INCIDENT - it has a start, it ends, it deserves a COLOUR. `Reduced` is a
        PROPERTY - it is stable, often permanent, and it deserves a SENTENCE. We colour what CHANGED; we
        write what IS.
        Never paint `Live + Reduced` as a degradation. Two amber pills - one meaning "40% of probes lost,
        get up", the other "ping-only, it has been like that for six months, sit back down" - do not cost
        one colour. They cost THE CREDIBILITY OF AMBER: a pill that sometimes shouts and sometimes says
        ignore me stops shouting. That is crying wolf, performed by the product, at every poll.
        `degraded` survives for what it actually denotes: live WITH LOSS, never live with less reach.
        Out-of-capability fields are `not evaluated` - never blank, never a dash, no severity; they are
        outside the frame of the question (deliberately the same words as `exclude`).
        And the dividend: the ping-only screen's content is THE LIST OF WHAT THE SOURCE CANNOT SEE, because
        that is where the action is - "I could grant it the raw-socket privilege and it would read
        addresses". Reduced scope is a capability to UNLOCK, not a fault to repair: the only place in this
        product where "I see less" is an invitation.
      F47 (SETTLED — the last open item of the glossary): `ignore` / « ignorer » -> `exclude` / « exclure ».
        NOT for the tone. "The tone disdains" is true and soft. The structural reason: `ignore` describes
        the OPERATOR'S ATTITUDE; every other verb in this grammar describes the OBJECT'S STATE or the
        relation between two records. It is the only one that names a human feeling - it is OUT OF GRAMMAR.
        And it contradicts the invariant: a tool that may not have an opinion about the backlog cannot carry
        "I don't care" in its action bar. `exclude` UNIFIES rather than replaces: an excluded object and an
        out-of-capability field are the same idea - outside the frame of the question. One concept, one
        term. Applied across 9 sites; `ignore` added to the retired-words denylist.
        EXTENSION, flagged (Guy to confirm): the keyboard binding was `I` for Ignore - a mnemonic. The word
        goes and its justification goes with it. Proposed `X` for Exclude (free, universally read as "out of
        view"). Note `E` was never a mnemonic: it is Superhuman's archive key, adopted for muscle memory.
        >> WITHDRAWN 2026-07-17 by Guy. Recorded, not deleted - annulment is an addition (D21). TWO reasons,
        >> and the second dissolves the question rather than answering it:
        >> (1) `X` APPLIED THE CRITERION THAT HAD JUST DIED. This same entry states that `E` was never a
        >>     mnemonic. So the criterion for a binding is MUSCLE MEMORY OF THE CORPUS, not evocation - and
        >>     "which letter suggests exclude?" is the exact question that `I` was the wrong answer to.
        >>     Proposing `X` re-committed the error the rename had just corrected. On the real criterion, `X`
        >>     is probably a COLLISION: in Gmail - the corpus `E` and `J` come from - `x` SELECTS.
        >>     (To verify before any binding is written, not asserted here.)
        >> (2) THERE WAS NO BINDING TO REPLACE. Verified against the body of this spec on 2026-07-17: the
        >>     letters E/G/S/I APPEAR NOWHERE IN IT. The body enumerates GESTURES (amber Document + ghost
        >>     Accept-gap/Snooze/Exclude/Attach) and says "kbd hints" exist; it never assigns a letter. The
        >>     only two letters in the document, `E` and `J`, sit inside a Superhuman INSPIRATION quote, not
        >>     a specification. `I` was never specified. **The line above ("keyboard bindings E/G/S/I")
        >>     describes an addition to a list that does not exist.**
        >> ==> NON-SUBJECT, and this is the durable statement: THIS SPEC DELIBERATELY DOES NOT ASSIGN
        >>     KEYBOARD LETTERS. It fixes the GESTURES and the CRITERION; the letters are written when the
        >>     inbox screen is written, for all six gestures AT ONCE, against the corpus - never one hole at
        >>     a time. A letter chosen in isolation is a letter whose neighbourhood nobody tested.
        >> (F56, fourth instance in 24h - reasoning from the map instead of the terrain. This one is the
        >>  orchestrator's twice over: it wrote the editHistory line, then built a standing question on it.)
      F42 + F44 (ADDITION): "What the Product Does Not Know - the Radar, not the Blank Field". The
        assumption the whole screen rests on - the gap is the CONTENT, abstention the EXCEPTION - was never
        measured and is not derivable from any spec: what fraction of a real network falls in each case is a
        property of the terrain. MEASURED: single-interface comfortably above the 80% threshold. IT HOLDS. The
        hierarchy stands; the grouped view stays a bootstrap MODE and does not become the product.
        Abstention is displayed AS A MAP: "I don't know" is pathetic when it is an admission of failure and
        trustworthy when it is a map - the difference is not the number, it is whether the cause is
        actionable. Nobody finds a radar pathetic: it never claims to see everywhere, it tells you where it
        sees, and that honesty is what makes you believe the lit area. "I don't know" is a MOTIF, never N
        failures - 96 multi-interface devices is ONE question. The counter measures the product's REACH, not
        the operator's debt: six months of inaction and it still reads the same number, same grey, same
        dignity. The six-month test applies to abstention BEFORE it applies to the backlog.
      F45 (ADDITION, Growth — placed against the bans on purpose): "The Missing Half - the Narrator".
        Every rule on the bans page is negative. It is a complete ethic and it is entirely a refusal: the
        product knows with surgical precision how not to be unpleasant, and has no idea how to be good.
        The six-month test carries the blind spot in its own formulation - it is a LOWER BOUND. An empty
        text file passes it perfectly.
        The missing symmetric: if the operator does nothing for six months and re-opens the tab, does the
        product TEACH him something? Six months of life happened on that network; the journal knows all of
        it and says nothing, because saying it would look like a reproach. BY DESIGNING THE REFUSAL TO
        JUDGE, WE DESIGNED A PRODUCT THAT REFUSES TO TELL.
        Our own rule, of which we only wrote the left half: "only a new fact may trigger a notification" ->
        A NEW FACT IS A REASON TO SPEAK. "This device has not been seen for four months" is a fact; it is
        not "you have not done anything in a while". The first speaks about the OPERATOR, the second about
        the NETWORK. THE LINE: the narrator reports facts about the network, never elapsed time about the
        operator. Any sentence whose subject is the operator is a nag and is banned.
      F50 / F51 (MEASUREMENTS folded in, not assumed): the ephemeral-interface lifecycle carries a large share of the
        known inventory (nearly half) - not a footnote, size the views for it. Hostname - one of grouping's
        three signals - is unusable on nearly half of known clients (absent 71, empty 11, NEVER null): the
        abstention floor is set by DATA AVAILABILITY, not by engine quality. An abstention target that
        ignores this measures the network, not the product.
      NOTE — the frozen banner was already specified "neutral, cool, desaturated, not scarlet". The
        grey-not-red instinct for `blind` was already right and is untouched: "I have no news" is not "it is
        broken".
  - date: '2026-07-16'
    source: 'architecture workflow (steps 1-4) — feedback items F11, F12, F13, F16'
    changes: |
      F16 (CORRECTION, urgent) - the Tailwind implementation note described a v3 API that no longer
        exists: v4 removed the JS-config `content` and `safelist`. Rewritten to `@source` /
        `@source inline()`. The decision was right; the syntax was dead, and someone would have
        implemented it in good faith. Added the v4 trap the CI drift-check does NOT catch: a class
        built in Rust and missing from `@source inline()` regenerates identical CSS, passes the diff
        green, and ships a colourless status pill — a silent product bug on a product whose core is
        the visual distinction between observed and declared.
      F11 (CORRECTION) - bootstrap is a MODE, not an onboarding. Filing it under "first run" was a
        design error: the wall recurs on every large migration. Regime 3 is regime 1 recurring.
        The baselining flow stays available for life, gated by VOLUME, never by a `first_run` flag.
        The grouped-view switch is auto-detected, reversible, and carries no judgement. Holding back
        by STRANGENESS and grouping by VOLUME are two mechanisms, not one.
      F12 (ADDITION) - the granularity of a question follows the regime. The question never
        disappears; its shape does: the FIELD in steady state, the MOTIF in bootstrap/migration
        (day-one divergences are systemic, not random: twelve uppercase hostnames is ONE decision).
        In bulk, a question is counted, not displayed. Same number of recorded decisions, ~100x fewer
        human gestures. Never withhold a question during a migration - that is when context is
        freshest; deferring converts a resolvable doubt into an unresolvable one.
      F13 (ADDITION) - the backlog bans as testable rules, each with its test, plus the decisive
        six-month test. The tool may make the backlog exist; it may not have an opinion about it.
      ...
      F14 (REFACTOR) - the canonical glossary is rebuilt as a binding EN/FR table mirroring the PRD.
        ...
        undoing is an ADDITION). Grammar: you document a VALUE, you accept a GAP.
        AND THE HOLE IN OUR OWN GATE, NAMED: the glossary uniqueness test checks that a term has one
        TRANSLATION, not one MEANING - which is exactly how `accept` came to name two opposite gestures
        without any test firing. Extended with a denylist of retired words.
      RENAME (Guy's decisions, applied across ~76 sites):
        ...
        covered BOTH the user pressing Undo AND the server failing. An undo is not a failure - the
        operator did not get anything wrong, he changed his mind. So: Undo -> back to `in_queue`
        (nothing failed, nothing to report); server failure/timeout -> `failed`. Naming the undo path
        "failed" would put a failure badge on a working product.
        accept-gap added wherever gestures are enumerated (card actions, keyboard bindings E/G/S/I,
        component states, glossary).
...
```mermaid
flowchart TD
  A[Fresh install / first launch] --> B[Setup wizard - resumable if abandoned]
  B --> C{Discovery source?}
  C -->|UniFi| D[Enter controller URL + API key]
  D --> T[Test connection]
  T --> T1{Reachable + auth + read scope?}
  T1 -->|No| T2[Typed hint: unreachable / 401 / 403 - fix and retry]
  T2 --> D
  T1 -->|Yes| T3[Green check: controller reachable, 84 clients in view]
  C -->|No UniFi| E[Declare subnets to scan]
  T3 --> F[First discovery cycle]
  E --> F
  F --> F1{Anything found?}
  F1 -->|Nothing| F2[Diagnostic: I could not reach anything - check key / subnet / firewall]
  F2 --> C
  F1 -->|Yes| G[Inbox pre-filled: N observed, 0 declared]
  G --> H[Bulk baseline: adopt current state]
  H --> I{Anomalies?}
  I -->|Yes| J[Held back to single triage]
  I -->|No| K[Baseline documented - idempotent, resumable if interrupted]
  J --> K
  K --> L[Name ~10 key devices]
  L --> M[First value: living inventory + first gaps]
```
...
```mermaid
flowchart TD
  A[Open inbox: priority-sorted queue] --> B[Focused card: object then gap then action]
  B --> C{Ambiguous?}
  C -->|Yes| R[Resolve: candidates + evidence + Compare]
  R --> N
  C -->|No| D{Action}
  D -->|Document E| E[Optimistic: card retracts, next mounts, counter--]
  E --> P[State: pending_commit, server deadline t+5s]
  P --> U[Undo toast + undo history]
  U -->|Undo| V[in_queue: card restored, pending cancelled - nothing failed]
  P --> G{Server result before deadline}
  G -->|OK| H[committed]
  G -->|Fail / timeout| I[failed: re-inserted at top, badge, NOT committed]
  G -->|Already handled elsewhere| X[409 / Gone: card removed cleanly, single commit]
  D -->|Snooze S| S[Defer; returns re-fetched at time]
  D -->|Exclude X| J[Out of scope, will not nag]
  D -->|Attach / Create| K[Link or new record]
  H --> N{Queue empty?}
  J --> N
  K --> N
  S --> N
  N -->|No| B
  N -->|Yes| Z[Inbox zero: calm summary]
```
...
```mermaid
flowchart TD
  A[Alert: unknown device / conflict / stale IP] --> B[In-app + webhook, carries deep link]
  B --> C[Tap link on phone]
  C --> D{Authenticated?}
  D -->|No / expired| E[Login then resume where you left off - decision preserved]
  D -->|Yes| F[Land on exact object, focused view]
  E --> F
  F --> AU[View auto-updates via polling]
  AU --> S1{Object state}
  S1 -->|Live| G[Just-enough evidence: which device, source, freshness]
  S1 -->|Resolved elsewhere / another tab| Q[Shows: already resolved - here is the decision]
  S1 -->|Deleted| Tb[Tombstone: gracefully closed, no dead-end]
  G --> H{Decision}
  H -->|Document or Resolve| Ia[One tap, resolved]
  H -->|Not now| Sn[Snooze]
  H -->|Investigate| Kd[Open on desktop: full record]
  Ia --> L[Undo available]
```
...
```mermaid
flowchart TD
  A[Discovery cycle for a source fails] --> B{Source unreachable > N cycles?}
  B -->|No| C[Retry silently]
  B -->|Yes| D[source_state = blind]
  D --> E[Frozen banner: source unreachable, state frozen, X min - nothing is lost]
  D --> F[Suppress observation-derived alerts for that source]
  D --> G[Keep last-known state - never fabricate device-gone or divergences]
  D --> H[Do not touch in-flight pending_commit items]
  E --> I{Source returns?}
  F --> I
  G --> I
  I -->|Yes| J[source_state = live: one reconciliation re-scan, not an avalanche]
  J --> K[Banner clears; new genuine changes surface normally]
```
...
| **Lift the doubt of an ambiguity** | **resolve** | **résoudre** | The gesture of the `ambiguous` state: the operator says whether the candidates the engine refused to guess between are **one machine or several** — two answers of one gesture, *the same machine* and *distinct machines*. Each is kept as an OPERATOR decision the engine never supersedes (D14); *the same machine* is the input of the device grouping (story 6.14c). Named before it acted (story 6.14), it acts with story 6.14b. It is **not** `attach`: no device record exists to attach to, and the answer is symmetric. ⚠️ **Not a conflict's gesture**: two sources disagreeing about one field are answered by *document* at field level. ⚠️ FR33 also lists *resolve* among an alert's actions; that use is the alerts epic's to reconcile with this row |
