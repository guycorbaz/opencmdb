...
- [x] [Review][Decision] ✅ **GUY 2026-08-22 — option (b): refuse the alias.** `Event::Alias` reds the gate, with a located probe. Cost measured NIL: `app.yml` carries zero anchors and zero aliases today (the `*` hits are emphasis inside comments). Taken over (a) resolving anchors — *an enumeration cannot claim the completeness of a property* (story 5.12), and (a) would leave `&` on `MappingStart`, whole-block anchors and multi-document anchors each to be handled in turn — and over (c) stating the limit, whose 5.12 precedent existed because a real closure was out of reach; here it costs four lines. **A YAML alias walks a retired term past the gate, and `completeness()` is structurally blind to it** — MEASURED: an anchor `tmpl: &shared "Merge alerts"` aliased into `nav.alerts` gives `✅ copy-vocabulary  no retired term in 289 key(s)`, nine gates green, 720/720 green, and the booted binary serves **`Merge alerts` in the French navigation**. `Event::Alias` falls into the `_ => {}` arm; an alias increments none of `scalars`/`keys`/`values`, so `keys + values == scalars` holds exactly as before — the counter whose doc calls it *"the only thing standing between that and a silent hole"* cannot see this class by construction. Control: the same term inline reds at `app.yml:344`. Carrier 2 catches it only for top-level keys. Options: (a) resolve anchors and lint the alias; (b) refuse `&`/`*` in `app.yml` outright — the strongest and the cheapest; (c) state the limit as a tripwire on story 5.12's precedent. [`xtask/src/copy_vocabulary.rs:224`, `:154`]
- [x] [Review][Decision] ✅ **GUY 2026-08-22 — option (a): redo the pass and write it.** Ten screens × two locales plus the six error bodies, against a rebuilt binary and a live store, read against the UX spec's five Microcopy Rules (`ux-design-specification.md:1444`). The ENGLISH column is the one never read, and it is this story's subject. Second consecutive story with this gap (6b.9's review found it too) — doing it is what breaks the pattern. **AC5's browser review has no artefact, and one cannot be written after the fact** — arbitration 5(b) is *"left to review, and the review is written down"*; T9 spells out the deliverable (which screens, which locale, which build, what was read against the five Microcopy Rules). The record holds one line and three defects. The three fixes are real and in the diff, so a look happened — what is missing is the artefact the arbitration was taken to produce. Rule 5, the one §0b calls the one this interface most needs, appears nowhere. Same finding as 6b.9's review, one story later. Options: (a) redo the pass screen by screen and write it; (b) write honestly what was actually done and register the remainder; (c) record AC5 as PARTLY MET with its reason. **Fabricating the record retroactively is not an option.**
- [x] [Review][Decision] ✅ **GUY 2026-08-22 — option (a): one key, both sites.** `deny()` and `challenge()` both take it, and the guard's perimeter widens to `auth.rs`. The arbitration's criterion is *"served at those same addresses"* and this body is, before every other. `/metrics` receives the French string — a body, not a header, inert for a machine client. Taken over (c) splitting screens from machines, which does not fall where the audience does: the pair-unconfigured refusal goes through `deny()` and a HUMAN reads it on `/triage`. **Is the 401 body inside arbitration 2(a′)'s perimeter?** — MEASURED on the live binary with `OPENCMDB_LOCALE=fr`: `401 Unauthorized` / `authentication required`, an English literal served **at every one of the ten addresses, before any screen**, on every request until credentials are supplied. It is the first body an operator on a closed deployment reads. The arbitration says *"the ten screens' rendered surface plus the six 500 bodies served at those same addresses"* — this is served at those same addresses. Options: (a) key it; (b) written exclusion at the site. [`crates/opencmdb-bin/src/auth.rs:84`]
- [x] [Review][Decision] ✅ **GUY 2026-08-22 — option (B): a per-handler budget, on `/diagnostic`'s precedent.** One shared constant and two `tokio::time::timeout` wrappers; the scan pass and the resolver are not touched. ⚠️ **The guard must be written against the instinct**: 6b.9 paid once for a test pool whose `acquire_timeout` expired BEFORE the budget, leaving the timeout arm dead under test. Taken over (A) `acquire_timeout` on the production pool — it governs the wait for a FREE connection, so under load a legitimate wait becomes an error, and nothing here measures the right value — and over (C) registering it, which would leave two screens holding the browser half a minute on the default every new deployment starts from. **`/triage` and `/dashboard` hold the browser 30.00 s before the new French sentence appears** — MEASURED against a real MariaDB paused mid-session: `/triage` 500 in 30.002731 s, `/dashboard` 500 in 30.002711 s, `/diagnostic` 200 in 2.003674 s (6b.9's budget). The copy is right and genuinely served; nothing measures *when*. The guard calls `server_error(PoolTimedOut)` directly, so it exercises the formatting and never sqlx's default `acquire_timeout` — correct about what it tests, unable to reach the delay. Pre-existing default, newly given a calm sentence. Options: (a) put the two screens on a budget like `/diagnostic`'s; (b) register with the story that owns the pool. [`crates/opencmdb-bin/src/page.rs:1201`]
- [x] [Review][Decision] ✅ **GUY 2026-08-22 — option (a): `merger` AND `mergers` join the `en` denylist**, with a located probe. 🔴 **And this row's first half was REFUTED by measurement before it reached Guy**: `gesture.accept_gap` renders `"Accept the gap"` — the glossary phrase EXACTLY — so `trim_end_matches(" the gap")` is a NO-OP on all five rows, not a bent oracle. The blind layer inferred *"the copy and the glossary already disagree and the oracle was bent to fit"*; the copy agrees. Deleting the trim is unambiguous and strictly strengthening → **reclassified as a patch**. ⚠️ A second claim of the reviewer's was wrong and is corrected here: adding `merger` does NOT require re-basing `the_separators_of_this_file_are_word_boundaries`, which tests the MATCHER (`contains_word("merger", "merge") == false`, still true) and never the denylist. **The glossary axis, on two points that are Guy's to arbitrate** — (i) `every_glossary_gesture_is_rendered_with_the_glossary_words` trims `" the gap"` off **both** operands, so `"accept"`, `"accept the gap"` and `"accept the gap the gap"` all compare equal: for the one multi-word row the test no longer detects the divergence it exists for, and no comment explains the weakening while every other deliberate weakening in this diff is justified in prose. (ii) The `en` denylist carries `merge`, `merges`, `merged`, `merging` but **not `merger`** — and `the_separators_of_this_file_are_word_boundaries` pins `merger` as *not* a hit, so an English UI string reading "Merger" passes the gate that exists to retire that word in English. It is the spelling a translator copying the French value produces. Adding it means re-basing that probe on another word. [`crates/opencmdb-bin/src/state_vocabulary.rs:340`, `xtask/src/copy_vocabulary.rs` `RETIRED`]
...
- [x] [Review][Defer] The `file-size` gate stops counting at the FIRST `#[cfg(test)]` — a probe file with `#[cfg(test)]` on line 2 and 2 500 `pub fn` lines below reports `✅ 40 file(s) under 2000 code lines`. Pre-existing; newly load-bearing because this story adds `state_vocabulary.rs` (first `#[cfg(test)]` at line 59, gate sees 58) and grows `main.rs` by 320 lines (3 529 lines, gate sees 30) [`xtask/src/main.rs:107`] — deferred, pre-existing

#### Dismissed, each with the check that dismissed it

- **`store_unreachable_body`'s doc contradicts `render_error_body`** (blind) — REFUTED by reading `render_shell`: its fallback IS a plain literal, `String::from("<!doctype html><title>opencmdb</title><p>render error")` at `page.rs:106`, exactly as the doc says. The layer confused it with `render_error_body()`, which is a different function serving a different failure. The sentence is true.
- **`set_locale` may not apply the primary-subtag rule that `t!(locale = …)` does** (blind) — REFUTED by booting: `OPENCMDB_LOCALE=fr-CH` serves *Tableau de bord* on `/triage`.
- **The security closure oracle's `paths.join(" · ")` disjunct accepts an empty value in half the matrix** (blind) — REFUTED by mutation: rendering the secrets row as `String::new()` reds `every_sentence_the_security_group_can_render_is_pinned_in_both_languages` at `diagnostic.rs:1139`; the non-empty-paths postures reject it and one red is enough.

#### Refuted by the layers themselves, recorded so nobody re-chases them

`build.rs` really does close the `app.yml` invisibility (changed the file alone → recompile in 1.35 s, `grep -a` finds the sentinel). The gate body is NOT deletable with the xtask suite green (3 red). The gate cannot be silently unwired from `run_ci` (the module-doc test reds). A sequence does not desynchronise the collector for the rest of the file. A multi-document `app.yml` is loud — by the compiler, 271 errors. A nested key missing its `fr` half DOES red. The attribute guard covers subdirectories. `opencmdb-core` untouched, no migration, `epics.md` and the UX spec not edited, no new crate. The twins are **byte-identical** (7851 chars each). `cargo fmt --check` and clippy clean, read from a file rather than a pipe. **697 → 720 verified on BOTH terms**, the baseline rebuilt in a separate worktree.
