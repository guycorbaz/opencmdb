...
- 2026-07-20 — CI (29739441887) FAILED: CI's runner does NOT permit unprivileged ICMP (`ping_group_range` restricted), so `Client::new()` returned `Permission denied`; and `poll` opened the socket BEFORE checking cancel, so `cancelled_poll_emits_nothing` got `Misconfigured` not `Cancelled`. FIX: check `cancel.is_cancelled()` at the very START of `poll` (a pre-cancelled poll opens no socket) — also more correct. Also added the `subnet_hosts` CIDR-expansion helper + `from_cidr` (the task claimed CIDR parsing; now honest) with a bounds test. Re-pushed (`de964f1`); real GitHub CI run green (29739756354). Status → done.
