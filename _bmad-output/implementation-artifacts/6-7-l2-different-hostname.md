...
Status: **done** — PR #137 squash-merged 2026-08-30 as `7b329d8`, after a CI run green **on the
head commit itself** (`782c5c7`, run `33314266141`, 2m30) — read from `gh pr checks` and NOT
from the watcher, which was **killed and emitted nothing**, for the second time in one session.
Implemented AND code-reviewed 2026-08-30 against a live `mariadb:10.11.11`
...
| 2026-08-30 | **MERGED.** PR #137 squash-merged as `7b329d8`; the branch was deleted on both sides by `delete_branch_on_merge`. ⚠️ The CI verdict was read from `gh pr checks` because the background watcher was **killed and emitted nothing — twice in one session**. That is a DIFFERENT failure from the one the register records (a watcher exiting 0 over zero checks): same symptom, nothing to read, different cause. In this environment, reading the conclusion yourself is the normal path, not the fallback. |
