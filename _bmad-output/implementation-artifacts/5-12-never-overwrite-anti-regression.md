...
Sixteen mutations, **sixteen reds, every one carried by a NAMED assertion; zero compiler-carried and zero
...
| M23 | the offset→line map back to one entry per CHARACTER | 1 red — and **the located corpus does not catch it**, measured: only the dedicated multibyte test does |
| M24b | the Rust allowlist keyed on the function NAME alone | 2 red — the dedicated test and `e33`; **no legitimate site broke** |
| M25 | the sanctioned path compared unnormalised | 2 red — a raw path drops the sanction and reds the two REAL adapters |
| M26 | the escaped-quote sentinel removed | 1 red — `e35`, and the control `e36` stays red |
| M27 | raw-string handling removed | 1 red — `e34`, control still red |
| M28 | the widened invisible ranges reverted | 1 red — `e37` |
...
#### 🔴 A defect the corpus was structurally blind to, and what it cost the corpus

Found by READING, after the repair passed its own mutation pass — by the second session launched on
this same story in parallel, and **re-measured here before being believed**.

`normalise_sql_text` built its offset→line map **one entry per CHARACTER** while
`authorship_findings` indexes it with a **byte** offset, and `to_ascii_lowercase` leaves non-ASCII
intact. Any string literal carrying a multibyte character shifted every later finding by
(bytes − chars). Measured: a 50-emoji literal on line 1 and a write on line 2 → **the gate reports
line 0**. A line that does not exist; and with smaller drift the result is worse, because it names a
real line and a wrong one.

**Scope, stated so the record does not overclaim**: this is **not** a detection hole.
`is_table_reference`, `statement_before`, `statement_after` and `enclosing_fn` all work in bytes over
`text` and stay correct — the finding reds normally. What was wrong is **where the gate says to
look**, and a gate that sends the reader to the wrong line spends the trust it has just earned.

🔑 **Why neither the probes nor the ten mutations could catch it**: `AUTHORSHIP_PROBES` pinned a
BOOLEAN — *does it red* — never *where*. And `e23_multibyte_line`, the corpus's own multibyte probe,
**reported the right line while the map was broken**: its drift is a dozen bytes and its line holds
enough entries to absorb it. Luck, not coverage.

So the corpus changed with the fix: **29 boolean verdicts became 29 located verdicts**
(`Option<usize>`), and `e23` now measures the thing it was written for. The one-line repair is
`for _ in 0..ch.len_utf8()`, and **M23** — reverting it — reds exactly one test, the dedicated
`the_reported_line_survives_a_multibyte_literal`, whose literal is long enough that luck cannot
absorb the drift. The located corpus alone does NOT catch it, and that was measured too.

⚠️ **The general lesson, worth more than the defect**: *a pinned boolean proves THAT a gate fires and
never WHERE.* That is a limit of the measuring apparatus, not of the gate — and it sat next to the
gate's own two limits without anyone writing it down.

#### 🔴 A hole INSIDE the narrowed promise: the allowlist sanctioned a spelling, not a place

The third finding of the parallel session, re-measured here before being believed — and the only one
of the three that sits **inside** the class the gate claims to hold.

`SANCTIONED_FNS` matched the enclosing function's NAME, wherever that function lived. Measured on the
committed tree: a new file under `crates/opencmdb-bin/src/` holding nothing but
...
character, no comment, no trick: the name was enough.

🔴 **Why this outranks an evasion.** It is not an adversary's probe, it is the ordinary gesture:
someone copies the adapter into another module, or writes a second one and gives it the name the job
already has. No intent, no cunning, and the gate is silent — while §12's *"What it promises"* puts
exactly that case on the held side (*"a future story will not add such a write by accident"*). A hole
beside the promise is a stated limit; a hole inside it is a false sentence.

🔑 **The apparatus had already said so itself**: the data-file half was keyed by PATH while the Rust
half was keyed by name, and all three were called *"sites"* — a word the code checked for one of
them. `SANCTIONED_FNS` and `SANCTIONED_FILE` are now one `SANCTIONED_SITES: [(&str, Option<&str>); 3]`
— a path, plus the enclosing `fn` when the file is Rust — and `authorship_findings` takes the
displayed path instead of a `bool` the caller had to know the meaning of.

`e33_sanctioned_name_other_file.rs` carries it, and it is a **different axis** from the other
thirty-two: they are planted in an unsanctioned file and ask *does it red*; `e33` carries the
sanctioned name and asks whether the name alone lets a write through. Two mutations bound it:

- **M24b** — the Rust half keyed on the name alone, exactly as shipped: **2 red**, the dedicated test
  and `e33`. **No legitimate site broke**, which is what the mutation was run to find out.
- **M25** — the path key kept but compared unnormalised (`p.display()` rather than the gate's own
  `shown`): **2 red**. That is the trap the parallel session flagged without measuring — a raw path
  drops the sanction and reds the two REAL adapters. Now measured, and the test asserts the
  sanctioned paths exist and still declare their functions, so a sanction cannot quietly come to
  cover nothing.

#### 🔴 A second hole inside the promise: the `"` bound truncated the READ half

Fourth finding of the parallel session, re-measured here. Same family as the third — the gate silent
on the ordinary gesture — and it needed a CONTROL to be provable.

`statement_after` stops at the first `;` **or `"`**. In a `.rs` file a `"` can also be a quote
*inside the SQL*, and then the bound truncates the statement instead of ending it. Everything after
it — the predicates, which is exactly where provenance columns live — becomes invisible to FR13.

| file | measured before |
|---|---|
| `r#"SELECT … FROM declared_attribute WHERE note = "n" AND origin = 'manual'"#` | **GREEN** |
| `"SELECT … FROM declared_attribute WHERE note = \"n\" AND actor_id = ?"` | **GREEN** |
| **CONTROL** — the same read with no quoted literal | **RED** |

🔑 **The control is what makes the other two mean anything.** Without it the pair shows two green
files and no cause; with it the difference is one character. It is committed as `e36`, and under M26
and M27 the two quoted forms go green while `e36` stays red — a control that does not move is what
gives its neighbours' movement a meaning.

**Why it is inside the promise**: `\"` is how anyone writes a quote in a Rust string literal. A
developer appending `AND actor_id = ?` to an existing query loses the FR13 guard without ever
learning it existed. No intent, no cunning.

**The repair keeps the `"` bound, which is load-bearing** — without it a finding spans two literals
and the gate invents phantoms — and instead rewrites the quotes that are *inside* a literal to a
sentinel (`QUOTE_IN_LITERAL`) during normalisation. Both spellings Rust actually permits are handled:
the escape `\"`, and raw strings `r"…"` / `r#"…"#` with their hash count tracked so the terminator
is the real one. Bounded on the literal's FRONTIERS rather than on the character.

#### ⚠️ `is_invisible` was an enumeration, and an enumeration cannot claim a property

The same session measured `INS<U+FE0F>ERT` and `INS<U+2063>ERT` passing: `e32`'s class with
code points the list did not name. The ranges are widened ONCE — variation selectors, the invisible
operators, the bidi controls — and deliberately **not to exhaustion**. Chasing an adversary through
the code-point space is a race this gate's promise has already declined; the closure for that class
