...
⚠️ **Four of the six diverge from the story's predicted counts, and the divergence is the point of
reporting it.** M1 matched (predicted 1, observed 1) — the type-preserving variant is the one the
story names. **M2 predicted 2 → observed 4. M3 predicted 2 → 3. M4 predicted 7 → 5. M5 predicted 1
→ 7.** The predictions came from the validation agent's minimal implementation; the shipped tests
are more numerous and, for M5, no longer follow the mutation (its conclusion assertions compare
against the independent `CORPUS_*` literals, so they red instead of moving with it). Every
divergence is upward except M4, whose type-preserving form touches fewer tests than the widened-key
form the agent measured. _(An earlier version of this record tabulated the observed column and
commented only on M1, which reads as though the other five matched. The code review caught it.)_

**Two mutations added by the code review, both proven to red:**

| # | mutation | reds | classification |
|---|---|---|---|
| M7 | evidence left in argument order instead of sorted | **1** | assertion — `a_pair_decides_the_same_whichever_side_is_the_left_argument` |
| M8 | `Fact::Uplink { peer_mac }` treated as an identity key | **1** | assertion — `a_peers_mac_is_not_this_devices_identity` |
...
- **AC3 was discharged as a NEGATIVE requirement**, as the validated story required: no IANA-prefix
  ...
  trap is reddened.
- **28 tests added** (core 100 → 128). Workspace **281 → 309 = 135 + 128 + 46**, counted mechanically
  after the last edit — 25 from the implementation, **3 added by the code review** (evidence
  symmetry, the `Uplink` peer-MAC non-key, and the group-address pin).
...
**Register: eight requirements dispositioned, and only FOUR are CLOSED.** Appended under
`## Deferred from: story-5.5`, grouped by requirement and citing entries by TITLE. Closed: R1
(evidence), R4 (the version constant), R6 (canonical id), R8 (the `NoMatch` producer half).
⚠️ **R7 was reported CLOSED and the code review downgraded it to partial** — its owner clause was
discharged but the entry is TITLED after 5.3's two compiler-carried tests, which are unchanged, and
AC8's two offered options were neither taken. The weaker true sentence is now written, and it
concludes the test **cannot** be made behavioural: `Outcome::Abstained` carries no rule field by
design, so no mutation short of adding one can make its assertion fail. **Left open, with the measurement that says why:** R3 — "one verdict per
...
| file | change |
|---|---|
| `crates/opencmdb-core/src/identity/l1.rs` | **NEW** — the join, the two rules, `decide_pair`, the ruleset constant, and **28** tests. Post-review: evidence sorted, `keys_of` exhaustive over `Fact` (no `_` arm), `verdict_for_pair` narrowed to `pub(crate)`, the I/G-bit class documented |
...
| `crates/opencmdb-core/src/identity/cascade.rs` | **docs only** — `git diff --unified=0 master...HEAD` reports **17 hunks, every one a content change**. ⚠️ An earlier version of this row claimed "12 claims, and the other 5 hunks are `cargo fmt` reflow"; **that reconciliation was false and the code review measured it so** — `rustfmt --edition 2024 --check` on master's `cascade.rs` exits 0, so `cargo fmt` contributed nothing to this diff. The reproducible number is the hunk count; "claims" was a boundary I drew by hand and could not reproduce |
| `crates/opencmdb-core/src/lib.rs` | flat re-export of `identity::l1`'s public items + crate doc (*"asserts nothing about identity yet"*). Post-review: `join` and `verdict_for_pair` removed from the flat list, with the reason stated |
| `crates/opencmdb-core/src/score.rs` | **docs only** — `:285`, `:293` and (post-review) `comparable_fields`' `verdict_vector` bullet; code, `VerdictVectorEntry` and both `size_of` tests untouched |
...
| Date | Change |
|---|---|
| 2026-07-31 | **DONE — PR #57 squash-merged as `0ebd50f`** after a green CI run. `master` carries 309 tests (135 + 128 + 46) and six green gates. The story's own rule held to the end: it ended at `review` with the PR open, and `done` was the merge's business, not the code-review workflow's step-6 default. |
| 2026-07-31 | **Code-reviewed — three layers (Blind Hunter · Edge Case Hunter · Acceptance Auditor), ~30 findings, 6 HIGH after deduplication. All applied; PR #57 updated.** 306 → **309 tests**. 🔑 **Two findings arrived independently from two layers** — the evidence asymmetry and the self-pair — and that agreement is why they were fixed rather than argued. **The three code HIGH:** (1) **`keys_of`'s `_ => None` catch-all** — measured: adding `Fact::Uplink { peer_mac }` as an identity key left the **entire workspace green** while a switch merged with everything plugged into it. The match is now exhaustive over `Fact`, in the idiom `fixtures.rs` established (*"Exhaustive on purpose — no `_` arm"*), so the compiler forces the decision on the next address-carrying variant; pinned again by a test (M8, 1 red). (2) **Evidence was not symmetric** — `decide_pair(a,b) != decide_pair(b,a)`, same conclusion but unequal `Decision`s, so any downstream dedup would see one logical pair as two; evidence is now sorted, by the same reasoning that made the join's value a `BTreeSet` (M7, 1 red). (3) **A group address (broadcast/multicast) is treated as an interface identity** — the I/G bit names no interface, and `opencmdb-bin` says so in this same epic; three devices reporting `ff:ff:ff:ff:ff:ff` merge into one group, a FALSE MERGE the corpus can never catch (zero I/G-set MACs across 14 streams). **Guy chose to DOCUMENT and REGISTER rather than filter** — consuming a structural reading at L1 is what this story's own doc says L1 does not do — with the current behaviour pinned so it cannot change in silence. Owner: a decision at Epic 6's contexting. Surface narrowed on Guy's call: `verdict_for_pair` → `pub(crate)` (exposing it beside `decide` made `cascade.rs`'s *"its two rules never appear in one vector"* false), and `join` dropped from the flat re-export. 🔴 **Three of the six HIGH were about CLAIMS, and all three were mine** — the fourth consecutive story where the completion record over-claimed: `cascade.rs`'s corrected `evidence` doc dropped the qualifier and became false for a `Neutral`; the File List's "17 hunks − 12 claims = `cargo fmt` reflow" reconciliation was **refuted by `rustfmt --check` exiting 0 on master's `cascade.rs`** (all 17 hunks are content); and the Debug Log tabulated six mutation counts while commenting only on the one that matched, leaving **four divergences** unstated (M2 2→4, M3 2→3, M4 7→5, M5 1→7). All corrected here. **R7 was downgraded from CLOSED to partial**: its owner clause was discharged but its TITLE — 5.3's two compiler-carried tests — was not, and AC8's two options were neither taken; the weaker true sentence is now written, and it concludes the test **cannot** be made behavioural because `Outcome::Abstained` carries no rule field by design. |
| 2026-07-31 | **Implemented — status `review`, PR open.** `identity/l1.rs` is NEW: the join `join(&[Observation]) -> BTreeMap<(L2DomainId, MacAddr), BTreeSet<ObsId>>`, the two corpus rules (`l1-exact-mac -> Decisive`, `l1-distinct-mac -> Disqualifying`, no MAC -> `Neutral`), `decide_pair` — **the first caller of `decide` outside its own tests** — and `CURRENT_RULESET_VERSION = RulesetVersion(1)` with no `Default`. **25 tests added; workspace 281 -> 306 = 135 + 125 + 46** _(309 after the code review)_, counted mechanically after the last edit; six `xtask` gates green (`float-free` now walks 3 files), `fixtures/` untouched. **Prove-to-red: six mutations, 1 / 4 / 3 / 5 / 7 / 10 reds, every one assertion-carried and none compiler-carried.** M2 confirmed AC5's derivation by RUNNING it — `Opposes` yields `Abstained { AbsenceOfProof }` and names no rule. **M6 is the mutation validation added, and it earned its place: the gap-hunt agent measured 0 reds for it on self-referential tests; on the shipped tests it reds 10**, because the assertions restate the two rule ids as independent string literals. Register: **eight requirements dispositioned, five reported CLOSED (R1, R4, R6, R7, R8) — R7 later downgraded to partial by the code review, leaving FOUR — and three deliberately left OPEN with the measurement that says why** — R3 is trivially true at L1 (one verdict per pair, so no mutation reds it for the right reason, owner Epic 6), R2 ships as an assertion rather than a refusal (no reachable branch under AC6), R5 keeps its placeholder because L1 supplies no tie. **One finding reported rather than absorbed:** `xtask/src/main.rs:1821` predicted *"story 5.5 writes IP literals under the guarded subtree"* and the prediction **did not come true** — zero `Ipv4Addr`, zero dotted quads under `identity/` — so the verify-only classification was itself falsified by verifying, and the message was corrected. Also caught: the story's `E0560`/`E0609` figure is a 5.4-era measurement (6+2 on this tree, not 5+1) and was not propagated. Flagged forward: 5.6's `blocking_recall >= 0.999` reds the `float-free` gate. ⚠️ Honest note: the tests and the implementation were written together, so there was **no failing-test phase** on the initial write; the prove-to-red evidence is the mutation pass alone. |
