...
- **(d) — RECOMMENDED: a STRUCT with an `Option` route rather than a two-variant enum.**
  `Gesture { label_key, route: Option<&'static str> }`: every gesture must still say whether it has a
  route, so *"looks live and calls nothing"* stays **unrepresentable** — `None` renders as planned by
  construction — and **there is no unconstructed variant to red the lint**. 🔑 *The arbitrated
  PROPERTY survives; only its spelling changes.*
- **(a) ship `Planned` alone now, `Live` arrives with story 6.4.** Honest, no `#[allow]`, but the
  type carries one variant and *"declaring is forced"* is true of nothing until 6.4.
- **(b) `#[allow(dead_code)]` on the variant.** 🔴 **Refuse-worthy**: that lint is the only carrier of
  `Screen::ALL`'s exhaustiveness outside `cargo xtask ci`, and story 6b.3 spent a whole story
  building a test because it is silenceable. Silencing it deliberately, here, teaches the next reader
...
- ⚠️ **The operator still cannot DO anything after this story.** It converts *"a screen with nothing
  ...
  not in reach. Story 6b.4's review put that plainly and it is worth carrying rather than blurring:
  *a better-lit dead end.* **6.4 is the story that ends it.**
...
- [ ] **T2 — one partial, one key pair** (AC1, §0b): `aria-disabled="true"` and **never** `disabled`;
      the words as arbitrated; **no story number in the surface**
- [ ] **T3 — the five gestures in the detail pane**, in the mock's order, with `Merger`/`Résoudre`
      following the row's kind as the mock does
...
- [ ] **T6 — LOOK at the screen**, `OPENCMDB_LOCALE=fr`, against a live database. 🔴 **Say whether a
      browser was used** — four stories running have shipped without one, and this one ships a row of
      five controls
...
| # | Mutation | Prediction |
|---|---|---|
| M1 | give a gesture a route it does not have | ⚠️ **prediction unknown on purpose** — if nothing reds, the type is not forcing what §0a claims and T1 is not done |
| M2 | render a planned gesture with `disabled` instead of `aria-disabled` | the NFR25 guard reds. ⚠️ If it stays green there is no such guard, and §0b's arbitration is carried by prose alone |
| M3 | put the owning story's number in the rendered label | the guard reds — §0b forbids a calendar in the surface |
| M4 | give a planned gesture `--accent-document` | story 6b.1's amber guard reds and the count leaves zero (§0d) |
| M5 | delete the `fr` half of the new key pair | `every_key_carries_both_locales` reds |
...
- `_bmad-output/implementation-artifacts/6b-4-triage-screen-on-the-real-gap.md` — §0b (this story's arbitrated design), §0c (the premise), and the five defects only looking found
