...
- **There is a running binary.** `v0.6.0` is the release this tree cuts, published to Docker Hub as `gcorbaz/opencmdb` (⚠️ *until the tag lands, `0.5.1` is the newest image on the registry*; this line named `v0.2.0` until `v0.6.0`, four releases stale). It starts, connects to MariaDB, sweeps a CIDR you give it every five minutes, reads each host's hardware address and name, and serves **ten screens** — four fed by the real store, two mixed, four carrying labelled example content. It runs on a real Synology NAS, and everything that broke on the way is in the issue tracker.
- **That is not the product yet.** ⚠️ **This line said *"one page, one connector, no triage inbox, no IPAM, no alerts, no admin UI"* until `v0.2.0`, and four of those six had stopped being true.** ⚠️ **And the *"no hardware address at all"* clause was true until 2026-09-10 and is now false**: the connector reads one off the neighbour table, and one sweep of 46 hosts formed 45 interfaces — the identity engine runs on a real network for the first time. What holds: **one connector**; **no UniFi source**; a scan perimeter still set by an environment variable; and **it forms interfaces, not devices** — ⚠️ *since `v0.6.0` two cards of one machine answering to one name are shown as ONE question on the triage screen, and on `master` you can ANSWER it — the same machine, or distinct machines — which the engine then never asks again (story 6.14b); and a machine answered *the same machine* is ONE row of the inventory (story 6.14c) — provided its records carry a hardware address*. ⚠️ **The "no gesture" clause was true until 2026-08-26 and is now false**: the triage screen's *Nouveau* rows carry a live button that documents the observation, provided `OPENCMDB_DOCUMENT_ENABLED=1` is set — see `docker/.env.example`. Four of the five controls beside it are still labelled *not yet*. What now exists: a triage inbox on the real gap, a dashboard, an inventory, **an addressing plan you declare and the product audits against the network**, an applications table, a sources screen, an alerts screen, a self-diagnostic and a commissioning screen. ⚠️ **Four of the ten screens are wholly labelled example content and two more carry an example section beside real content** — measured off `Screen::nature`, not counted from memory — and the page says so on the page. *This line read "six of them" until the addressing plan stopped being an invented example network.* Epics 1–5, 6b and **14 (IPAM)** are done; **Epic 6 (grouping) is in progress** — it was frozen at story 6.7 on the measurement of three outside reviews, and unfrozen on 2026-09-23 once the connector read a hardware address.
...
**Stack:** Rust (edition 2024) · [axum](https://github.com/tokio-rs/axum) · [askama](https://github.com/askama-rs/askama) templates · [HTMX](https://htmx.org/) (committed, never a CDN) · **hand-authored CSS** on the reference mock's tokens (⚠️ *this line said Tailwind until `v0.2.0`; there is no Tailwind chain and no `cargo xtask css` — the decision was taken at story 6b.1 and re-confirmed at 6b.2, on the measurement that it generated nothing this UI uses*) · [sqlx](https://github.com/launchbadge/sqlx) · [tokio](https://tokio.rs/). Polling, not SSE, at MVP. Server-rendered; no SPA framework.
...
- For the generic scanner: nothing special. A container in its own network namespace pings as a non-root user out of the box — Docker sets `net.ipv4.ping_group_range` for it. **Do not use `network_mode: host`**: it inherits the host's value instead, which is empty on Synology DSM, and the scan then fails silently. ⚠️ **Hardware addresses ship now, and they change what the deployment needs**: the scanner reads them off the host's neighbour table, so **behind an ordinary bridge the host answers every ARP and the table holds the gateway alone** — the deployment then scans, records and names, and **never groups anything, silently**. Use `macvlan`, `ipvlan` or host networking if you want grouping; the administrator manual carries the trap. The UniFi connector needs neither.
...
Project-specific gates live in `cargo xtask ci` (in Rust, never in YAML) — the DDL collation gate, the retired-vocabulary check, and the fixture/artifact hash checks. `cargo xtask mutate` is the second subcommand, since story 6.4b — the mutation driver. There are no stubs.

> `cargo run` (with the database configured — `DATABASE_URL`, or the discrete `DATABASE_*` variables) starts an axum server serving **ten screens**; `/` is a **303 redirect to `/triage`**, the triage inbox showing the observed-vs-declared **gap**, reconciled from the declared records and the ingested observations. ⚠️ This read *"a single page at `/`"* until `v0.2.0`. Alongside it, a `/healthz` liveness probe and an authenticated Prometheus `/metrics` endpoint (scrape token via `OPENCMDB_METRICS_TOKEN`, `Authorization: Bearer …`). A deny-by-default auth layer refuses every other route: **the product is not publicly readable, and has not been since story 6.1**. Supply HTTP Basic credentials with **`OPENCMDB_BASIC_USER` and `OPENCMDB_BASIC_PASSWORD`** — both or neither; half a pair refuses to start, by name. With neither set, every screen answers `401` and no one can sign in, which is the deliberate posture of a fresh instance rather than a fault. The user half may not contain a colon (RFC 7617), and neither half may carry a non-ASCII or control character, since no browser dialog can type one. Set `OPENCMDB_SCAN_CIDR` (e.g. `192.0.2.0/24`) to run a one-shot ARP/ping scan on startup so the page shows genuinely observed state; **`OPENCMDB_DNS_SERVER` decides whether the triage queue reads `sw03` and `wifi01-grange` or a column of bare addresses** — it is the resolver asked for each host's name, and it should usually be your DHCP server (measured on the reference network: the configured system resolver, which is the default, named 2 of 69 addresses; the router that issued the leases named 37); `OPENCMDB_LOCALE=fr` renders the UI in French (`en` and `fr`, a region suffix such as `fr-CH` is accepted; **an unrecognised value now refuses to start, by name**, rather than falling back to English in silence); `OPENCMDB_LOG_DIR` enables daily-rotating file logs alongside stdout. **This is the walking skeleton, not the product** — see the status section above for what it is not.
...
- **Credential storage is PLANNED, and nothing is stored today** — there is no credential table and no encryption call site, and the product's own `/diagnostic` screen says so: *"stored credentials — none stored"*. When it arrives (Epics 10 and 19) it is designed so that stored credentials are encrypted at rest with an envelope scheme, the master key living **outside** the data volume. ⚠️ This line asserted that as CURRENT fact until `v0.2.0`; a test in the product forbids the same sentence from ever rendering on screen, because it is false.
