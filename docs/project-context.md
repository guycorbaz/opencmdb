...
counts on `master` after 5.7 merged: **153 (bin) + 153 (core) + 46 (xtask) = 352**, measured
with `cargo test --workspace --locked`. _(This line read "86 + 46 + 38" until
...
**The corpus harness is now fed** — `l1_runner` fills `score_corpus`'s map with 13 real engine
answers — but **the blocker still has no production caller** (it is reached from its own tests and
