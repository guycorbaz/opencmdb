...
\begin{planned}
The triage screen shows these five controls today, each labelled \emph{Not yet}:
none of them acts. They are visible so that the shape of the work is legible before the
gestures are built. Epic~7 makes them live.
\end{planned}
...
\begin{planned}
\emph{Document-all} exists as a route and is reached from no screen; \emph{document-field} is
not built. Epic~7 gives both a control the operator can press.
\end{planned}
...
control, named after its address. Releasing tells the plan to forget what the network has shown on
that address \emph{until now}: the entry leaves the list and, inside a \texttt{static} range, the
address can be offered again. Nothing the scanner recorded is deleted --- the release is kept as a
record of its own, beside the sightings.

A release lapses by itself when the network answers again. A machine seen on the address after you
released it holds the address once more, and the entry comes back as an ordinary finding --- an
address something answers on is never offered. A network card replaced after the release counts only
from the moment it was seen, so it is not reported as a conflict with the card it replaced.

An address you defined in the plan cannot be released: the plan never offers it anyway, and the
refusal says to remove the address instead. A release cannot be undone from the screen; if you
released a machine that was only asleep, its address may be offered until it is seen again.
...
\begin{planned}
VLANs and IPv6 are not implemented yet. Editing, deleting and releasing are described above and ship
with this release.
\end{planned}
...
\begin{description}[style=nextline,leftmargin=0pt]
  ...
  \item[Release] Stop the plan holding an address: what the network showed on it until now is
        forgotten, and the address can be offered again. A release is kept as a record, never a
