...
\subsection{One device, one row}
The \emph{Devices} screen lists what you added \emph{one row per device}. Records you answered
\textbf{The same machine} share a row, which names all their addresses and says how many records it
holds; so do records of one network card documented at two addresses. Only your answers join two
network cards --- a shared name, a shared address or a shared history never does: an address the
network hands to another machine later (DHCP) is not a proof that the two are one. The header counts
both: \emph{devices} and \emph{records}.

A record reaches its network card by the hardware address it holds, and a row's \emph{last seen}
is when the network last showed its cards. A record that holds no hardware address stays a row of its
own even after you answer \emph{the same machine}: one added before opencmdb read hardware addresses
(before \texttt{v0.5.0}), one added while opencmdb ran behind a Docker bridge (where it reads none),
and the record of the machine running opencmdb itself (which keeps no neighbour entry for itself).

\begin{planned}
Limits of this version. An answer cannot yet be changed or undone from the screen, and the reach
count says \emph{how many} questions you answered, not \emph{which}. Pressing \textbf{Add} on each
address of a machine you called one still writes two records --- they share one row on the
\emph{Devices} screen, but the triage screen still compares each record on its own. A question whose
machines are not answering on the network right now offers no answer, and says so; the answers
return with the next sweep that sees them.
\end{planned}
