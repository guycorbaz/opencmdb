...
A record reaches its network card by the hardware address it holds. A record added before opencmdb
read hardware addresses (before \texttt{v0.5.0}) holds none, and its sighting was placed on no card,
so it stays a row of its own even after you answer \emph{the same machine}.
