...
wrong rather than retyping the rest, and the same rules apply as when you defined it. A range may be
widened onto ground it already occupies; it may not be widened onto a neighbour's. These lists are
...
says that it holds them --- not that it does not exist. Before either removal fires, reaching its
control tells you what the removal would change: how many addresses the network shows inside that
range are explained by it today and would become findings once it is gone, whether it is the only
\texttt{static} range and removing it would leave nothing to offer here, and, for a subnet, how many
records are still holding it back. As with the other two warnings, it refuses nothing --- it says
what would change, and the removal stays available.
...
\begin{planned}
Editing, deleting and releasing an entry of the plan; VLANs; and IPv6 are not implemented yet.
\end{planned}
