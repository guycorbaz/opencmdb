...
\begin{planned}
⚠️ This block said installation would be documented \emph{``once the first release image is
...
\begin{description}[style=nextline,leftmargin=0pt]
  ...
  \item[\texttt{OPENCMDB\_DOCUMENT\_ENABLED}] Whether the documenting write route exists at
    all. ⚠️ It is reached from no screen today, so setting it changes nothing an operator can
...
\textbf{The product is closed by default and has been since \texttt{v0.2.0}.} Every screen
...
⚠️ With neither set, every screen answers \texttt{401} and nobody can sign in. That is the
...
\end{planned}
Source API keys and other secrets will be encrypted at rest. The encryption key is kept
\textbf{separate from the database volume}, so a stolen database file alone does not reveal
your source credentials. API-key rotation and an encrypted secret backup are planned with it.
