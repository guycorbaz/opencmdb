...
\begin{warning}
A special character in the password is mangled by \emph{two independent mechanisms}, at two
different moments. Confusing them costs hours, because both produce the same opaque
``access denied'' at startup.

\medskip
\textbf{1. A \texttt{\$} must be doubled: \texttt{\$\$}.} Docker Compose interpolates the
contents of your \texttt{.env} file, so a \texttt{\$} starts what it takes to be a variable
reference and the rest of the value is discarded. A password written \texttt{pa\$word} reaches
opencmdb as \texttt{pa}. This happens \emph{before} the process starts, so no amount of encoding
inside the application can recover it. Measured: \texttt{abc\$def} arrives as \texttt{abc};
\texttt{abc\$\$def} arrives as \texttt{abc\$def}.

\medskip
\textbf{2. URL-reserved characters must be percent-encoded.} Independently, the password sits
inside a URL: \texttt{@}~$\rightarrow$~\texttt{\%40}, \texttt{:}~$\rightarrow$~\texttt{\%3A},
\texttt{/}~$\rightarrow$~\texttt{\%2F}, \texttt{\#}~$\rightarrow$~\texttt{\%23},
\texttt{?}~$\rightarrow$~\texttt{\%3F}, \texttt{\%}~$\rightarrow$~\texttt{\%25},
space~$\rightarrow$~\texttt{\%20}. opencmdb percent-decodes the user info before connecting, so
MariaDB still receives the original password.

\medskip
Both at once: the password \texttt{s3cr\$t@x} is written \texttt{s3cr\$\$t\%40x}.
\end{warning}

\begin{warning}
The host in the URL is the address of your database server \emph{as seen from inside the
...
\begin{warning}
Run the first start in the \textbf{foreground} (\texttt{docker compose up}, not
\texttt{up~-d}). A fatal startup error is currently written to standard output only and does not
reach the log files, so a container that crash-loops under \texttt{restart: unless-stopped}
leaves nothing in \texttt{opencmdb.YYYY-MM-DD.log} but a column of identical
\texttt{opencmdb starting} lines.
\end{warning}
...
\addlinespace
\ldots\ and that host changes when you change the URL
...
\addlinespace
Log files contain only \texttt{opencmdb starting}
  & The crash reason went to standard output
  & Run in the foreground, or read \texttt{docker logs} \\
\bottomrule
\end{tabular}
\end{center}
