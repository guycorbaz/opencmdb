...
Measured on 2026-09-10, same LAN, same image: from a macvlan container a sweep of 46 hosts minted
\textbf{45 interfaces and placed 45 sightings}; the identity engine had, until that day, never run
on anything but fixtures.

\textbf{The failure is silent.} A bridge deployment scans, records, names hosts by reverse DNS and
shows you a working product --- it simply never groups anything, and \texttt{/sources} will say it
observes hardware addresses because that is what the connector is built to ask for.
\end{warning}
